...
-- NULL VALUE CHECKS 
SELECT
SUM(CASE WHEN customer_id IS NULL THEN 1 ELSE 0 END) AS count_null_customer_id,
SUM(CASE WHEN customer_unique_id IS NULL THEN 1 ELSE 0 END) AS count_null_customer_unique_id,
SUM(CASE WHEN customer_zip_code_prefix IS NULL THEN 1 ELSE 0 END) AS count_null_customer_zip_code,
SUM(CASE WHEN customer_city IS NULL THEN 1 ELSE 0 END) AS couny_null_customer_city,
SUM(CASE WHEN customer_state IS NULL THEN 1 ELSE 0 END) AS couny_null_customer_state
FROM customers;
...
SELECT
COUNT(DISTINCT customer_city) AS UNIUE_CITIS_COUNT,
COUNT(DISTINCT customer_state) AS UNIUE_STATES_COUNT
FROM customers;
...
WHERE customer_city IS NULL
   OR customer_city = ''
   OR customer_city REGEXP '^[0-9]'                    -- starts with a digit
   OR customer_city REGEXP '[0-9]$'                    -- ends with a digit
   OR customer_city REGEXP '^[[:alpha:]]+[0-9]+$'      -- letters followed only by digits
   OR customer_city REGEXP '^[0-9]+[[:alpha:]]+$'      -- digits followed only by letters
   OR customer_city REGEXP "[^[:alpha:]À-ÿ 0-9'-]"     -- invalid characters
   OR customer_city REGEXP '^[[:space:]]'              -- leading whitespace
   OR customer_city REGEXP '[[:space:]]$';             -- trailing whitespace
...
SELECT order_id, COUNT(*) AS occurences FROM orders 
GROUP BY order_id
HAVING COUNT(*)>1;
...
SELECT order_id, order_item_id , COUNT(*) AS ocuurences FROM order_items
...
SELECT oi.*, o.order_purchase_timestamp FROM order_items OI
JOIN orders O 
...
SELECT product_id, COUNT(*) TOTAL_ITEM_SALE FROM order_items
GROUP BY product_id
ORDER BY TOTAL_ITEM_SALE DESC;
...
SUM(CASE WHEN product_photos_qty IS NULL THEN 1 ELSE 0 END) AS pproduct_photos_qty_nulls,
...
SUM(CASE WHEN product_width_cm IS NULL THEN 1 ELSE 0 END) AS product_width_nulls
FROM products;
/*
 Interpretation:
...
SELECT product_id, COUNT(*) AS occurrences FROM products 
GROUP BY product_id
HAVING COUNT(*) > 1;
...
SELECT * FROM products
WHERE product_photos_qty IS NULL OR product_photos_qty=0;
...
SELECT seller_id, COUNT(*) AS ocuurences
FROM sellers
GROUP BY seller_id
HAVING COUNT(*) > 1;

--  UNIQUE CITY & STATE COUNT
SELECT COUNT(DISTINCT seller_city) AS unique_cities, COUNT(DISTINCT seller_state) AS unique_states
FROM sellers;
...
SELECT order_id, payment_sequential, COUNT(*) AS ocuurences FROM order_payments
...
SELECT * FROM order_payments 
WHERE payment_value > 0 AND payment_type IN ('boleto', 'voucher', 'debit_card');
...
SELECT order_id, COUNT(*) AS no_of_payments, SUM(payment_value) AS total_paid FROM order_payments
GROUP BY order_id
HAVING COUNT(*) > 1
...
WHERE geolocation_lat NOT BETWEEN -34 AND +5 AND geolocation_lng NOT BETWEEN -74 AND -34;
/*
 Interpretation:
...
SELECT geolocation_zip_code_prefix, geolocation_city, geolocation_state, COUNT(*) AS occurrences FROM geolocation
GROUP BY 1,2,3
HAVING COUNT(*) >1;
/*
 Interpretation:
...
SELECT review_id, COUNT(*) AS ocuurences FROM order_reviews
GROUP BY review_id
HAVING COUNT(*) > 1
ORDER BY ocuurences DESC;
...
SELECT odr.* , o.order_purchase_timestamp FROM order_reviews odr 
JOIN orders o 
on odr.order_id = o.order_id
WHERE odr.review_creation_date <  o.order_purchase_timestamp;

-- B. Review created BEFORE the product was delivered -> SUSPICIOUS
SELECT r.*, o.order_delivered_customer_date FROM order_reviews r
JOIN orders o 
...
SELECT * FROM order_reviews 
WHERE LENGTH(review_comment_message) < 5;

-- Extremely long reviews (spam-like)
SELECT * FROM order_reviews 
WHERE LENGTH(review_comment_message) > 300;
...
SELECT product_category_name, product_category_name_english, COUNT(*) AS occurences FROM product_category_name_translation
GROUP BY 1,2 
HAVING COUNT(*) > 1;
...
WHERE product_category_name REGEXP '[0-9]';

SELECT * FROM product_category_name_translation
WHERE product_category_name_english REGEXP '[0-9]';
...
SELECT * FROM product_category_name_translation
WHERE product_category_name REGEXP ' %'; -- WHERE product_category_name REGEXP '^[[:SPACE:]]'; IT INLUDE SPACE,NEWLINE AND AB

SELECT * FROM product_category_name_translation
WHERE product_category_name REGEXP '% ';

-- 6. CHECK CATEGORY NAMES NOT PRESENT IN PRODUCTS TABLE
-- (important for validating joins)
SELECT p.product_category_name FROM  product_category_name_translation pt
...
ON pt.product_category_name = p.product_category_name
WHERE pt.product_category_name IS NULL AND p.product_category_name REGEXP '[[:alpha;]]' ;
